...
async function migrateData() {
  try {
    console.log('🚀 Rozpoczynam migrację danych z SQLite do Neon PostgreSQL...');
    ...
    for (const block of blocks) {
      ...
      `, [block.id, block.name, block.description, block.delivery_date, block.total_floors, block.total_apartments, block.created_at]);
    }
    ...
    for (const floor of floors) {
      await pool.query(`
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          total_apartments = EXCLUDED.total_apartments
      `, [floor.id, floor.block_id, floor.floor_number, floor.floor_name, floor.total_apartments]);
    }
    ...
    for (const apartment of apartments) {
      await pool.query(`
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          created_at = EXCLUDED.created_at
      `, [apartment.id, apartment.block_id, apartment.floor_id, apartment.apartment_number, apartment.area, apartment.rooms, apartment.price, apartment.status, apartment.pdf_path, apartment.storage_pdf_path, apartment.created_at]);
    }
    ...
    for (const room of storageRooms) {
      await pool.query(`
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          area = EXCLUDED.area,
          pdf_path = EXCLUDED.pdf_path
      `, [room.id, room.apartment_id, room.storage_number, room.area, room.pdf_path]);
    }
...
// Uruchom migrację tylko jeśli jesteśmy w środowisku z dostępem do Neon
if (process.env.POSTGRES_URL) {
  migrateData();
} else {
  console.log('⚠️  Brak zmiennej POSTGRES_URL. Ustaw zmienną środowiskową i spróbuj ponownie.');
}
