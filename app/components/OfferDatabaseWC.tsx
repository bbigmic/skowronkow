...
const OfferDatabase = () => {
  ...
  const handleApartmentSelect = (apartmentId: number) => {
    ...
    if (!isInitializing) {
      updateURL('apartment', selectedBlockId || undefined, selectedFloorId || undefined, apartmentId)
    }
  }
  ...
  const updateURL = (view: string, blockId?: number, floorId?: number, apartmentId?: number) => {
    ...
    if (blockId) {
      params.set('block', blockId.toString())
    }
    if (floorId) {
      params.set('floor', floorId.toString())
    }
    if (apartmentId) {
      params.set('apartment', apartmentId.toString())
    }
  ...
  useEffect(() => {
    if (floor && !selectedBlockId) {
      setSelectedBlockId(floor.block_id)
    }
  }, [floor, selectedBlockId])

  // Ustawianie blockId i floorId na podstawie apartment z hooka useApartment
  useEffect(() => {
    if (apartment && (!selectedBlockId || !selectedFloorId)) {
      if (!selectedBlockId) {
        setSelectedBlockId(apartment.block_id)
      }
      if (!selectedFloorId) {
        setSelectedFloorId(apartment.floor_id)
      }
    }
  ...
  useEffect(() => {
    if (currentView === 'overview') {
      ...
      blocks.forEach((block, index) => {
        ...
        // Sprawdź czy blok jest disabled
        const isDisabled = block.name.toLowerCase() === 'blok 2'

        // Funkcje do zarządzania stanem hover (tylko dla enabled bloków)
        const setBlockHover = (isHovered: boolean) => {
          if (isDisabled) return // Nie obsługuj hover dla disabled bloków
        ...
        // Event listeners dla kształtu (tylko dla enabled bloków)
        if (blockShape && !isDisabled) {
        ...
        // Event listeners dla etykiety (tylko dla enabled bloków)
        if (blockLabel && !isDisabled) {
        ...
        // Event listeners dla etykiety daty (tylko dla enabled bloków)
        if (blockDateLabel && !isDisabled) {
          ...
          {currentView === 'apartment' && apartment ? (
            ...
            <div className="relative w-full">
              ...
              {currentView === 'overview' && blocks.map((block, index) => {
                ...
                // Sprawdź czy blok jest disabled (blok 2)
                const isDisabled = block.name.toLowerCase() === 'blok 2'
                ...
                return (
                  <div key={block.id}>
                    ...
                    <button
                      onClick={isDisabled ? undefined : () => handleBlockSelect(block.id)}
                      disabled={isDisabled}
                      className={`absolute border-0 transition-all duration-300 group ${
                        isDisabled 
                          ? 'bg-gray-300/20 cursor-not-allowed opacity-60' 
                          : 'bg-primary-500/20 hover:bg-primary-500/40 border-primary-500 cursor-pointer'
                      }`}
                      style={{
                        top: position.top,
                        left: position.left,
                    ...
                    <button
                      onClick={isDisabled ? undefined : () => handleBlockSelect(block.id)}
                      disabled={isDisabled}
                      className={`absolute bg-transparent transition-all duration-300 group pointer-events-auto ${
                        isDisabled ? 'cursor-not-allowed' : 'hover:bg-primary-500/10 cursor-pointer'
                      }`}
                      style={{
                    ...
                    >
                      <div className={`font-bold text-lg px-3 py-1 rounded shadow-lg transition-transform duration-300 pointer-events-auto ${
                        isDisabled 
                          ? 'bg-gray-500 text-white transform-none' 
                          : 'bg-primary-600 text-white transform group-hover:scale-110'
                      }`}>
                        {block.name}
                      </div>
                    </button>
                    ...
                    >
                      <div className={`backdrop-blur-sm font-medium text-xs px-2 py-1 rounded-md shadow-sm transition-all duration-300 ${
                        isDisabled 
                          ? 'bg-gray-300/80 text-gray-500' 
                          : 'bg-white/80 text-gray-700'
                      }`}>
                        {isDisabled ? 'Wkrótce w sprzedaży' : `Oddanie ${block.delivery_date}`}
                      </div>
                    </button>
                  </div>
                )
              ...
              {currentView === 'floor' && floorApartments.map((apartment, index) => {
                ...
                const getApartmentPosition = (apartmentNumber: string, blockName: string, floorName: string) => {
                  ...
                  const apartmentConfigs: Record<string, any> = {
                    ...
                    'blok1-parter-m1': { top: '40%', left: '10%', width: '15%', height: '40%', clipPath: 'polygon(45% 29%, 86% 29%, 86% 77%, 45% 77%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(20% + 7.5%)' },
                    'blok1-parter-m2': { top: '40%', left: '0%', width: '20%', height: '40%', clipPath: 'polygon(28% 19%, 71% 19%, 71% 29.5%, 84% 29.5%, 84% 77%, 28% 77%, 28% 57%, 10% 57%, 10% 27%, 28% 27%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(0% + 10%)' },
                    'blok1-parter-m3': { top: '10%', left: '1%', width: '20%', height: '40%', clipPath: 'polygon(15% 48.5%, 24.5% 39.5%, 66.5% 39.5%, 66.5% 48.5%, 83.5% 48.5%, 83.5% 55%, 94% 55%, 94% 94%, 24% 94%, 24% 86%, 15% 86%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(0% + 10%)' },
                    'blok1-parter-m4': { top: '10%', left: '15%', width: '20%', height: '40%', clipPath: 'polygon(24% 39.5%, 57% 39.5%, 57% 48.5%, 75% 48.5%, 75% 55%, 82% 55%, 82% 94%, 24% 94%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(20% + 7.5%)' },
                    'blok1-parter-m5': { top: '10%', left: '30%', width: '10%', height: '40%', clipPath: 'polygon(34% 39.5%, 97% 39.5%, 97% 94%, 15% 94%, 15% 55%, 34% 55%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(35% + 5%)' },
                    'blok1-parter-m6': { top: '10%', left: '35%', width: '15%', height: '40%', clipPath: 'polygon(32% 56%, 34% 56%, 34% 48.5%, 57% 48.5%, 57% 39.5%, 95% 39.5%, 95% 56%, 98% 56%, 98% 94%, 32% 94%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(45% + 2.5%)' },
                    'blok1-parter-m7': { top: '45%', left: '40%', width: '15%', height: '40%', clipPath: 'polygon(10% 30%, 33.5% 30%, 33.5% 17%, 18% 17%, 18% 7%, 67% 7%, 67% 64%, 10% 64%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(45% + 2.5%)' },
                    'blok1-parter-m8': { top: '50%', left: '30%', width: '20%', height: '40%', clipPath: 'polygon(15% 21%, 31% 21%, 31% 4%, 75% 4%, 75% 17%, 57.4% 17%, 57.4% 52%, 15% 52%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(35% + 5%)' },
                    'blok1-parter-m25': { top: '40%', left: '75%', width: '15%', height: '40%', clipPath: 'polygon(15% 28%, 55% 28%, 55% 76%, 15% 76%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(65% + 7.5%)' },
                    'blok1-parter-m26': { top: '40%', left: '80%', width: '20%', height: '40%', clipPath: 'polygon(29% 30%, 17% 30%, 17% 76%, 74% 76%, 74% 19%, 29% 19%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-parter-m27': { top: '20%', left: '78%', width: '20%', height: '40%', clipPath: 'polygon(90.5% 14%, 39% 14%, 39% 23%, 22% 23%, 22% 30%, 11% 30%, 11% 70%, 84% 70%, 84% 61.5%, 90.5% 61.5%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-parter-m28': { top: '10%', left: '68%', width: '15%', height: '40%', clipPath: 'polygon(38% 48%, 13% 48%, 13% 56%, 5% 56%, 5% 94%, 82% 94%, 82% 39%, 38% 39%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(65% + 7.5%)' },
                    'blok1-parter-m29': { top: '10%', left: '55%', width: '15%', height: '40%', clipPath: 'polygon(79% 39.5%, 79% 55%, 90% 55%, 90% 95%, 36% 95%, 36% 39.5%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(55% + 5%)' },
                    'blok1-parter-m30': { top: '10%', left: '50%', width: '15%', height: '40%', clipPath: 'polygon(43% 39.5%, 43% 48%, 67% 48%, 67% 94%, 3% 94%, 3% 55%, 6% 55%, 6% 39.5%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(50% + 2.5%)' },
                    'blok1-parter-m31': { top: '40%', left: '45%', width: '20%', height: '40%', clipPath: 'polygon(62% 29%, 62% 19%, 25% 19%, 25% 76%, 68% 76%, 68% 41.5%, 50% 41.5%, 50% 29%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(50% + 2.5%)' },
                    'blok1-parter-m32': { top: '40%', left: '50%', width: '20%', height: '40%', clipPath: 'polygon(69% 29%, 27% 29%, 27% 41.5%, 43% 41.5%, 43% 76%, 84% 76%, 84% 46%, 69% 46%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(55% + 5%)' },

                    
                    // BLOK 1 - PIĘTRO 1 (16 mieszkań) - ✅ POPRAWIONE ZGODNIE Z BAZĄ DANYCH
                    'blok1-pietro1-m9': { top: '40%', left: '15%', width: '20%', height: '40%', clipPath: 'polygon(9% 29%, 40% 29%, 40% 34%, 67% 34%, 67% 49%, 56% 49%, 56% 79%, 9% 79%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(40% + 10%)' },
                    'blok1-pietro1-m10': { top: '40%', left: '0%', width: '20%', height: '40%', clipPath: 'polygon(27% 20%, 71% 20%, 71% 31%, 84% 31%, 84% 78%, 27% 78%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(20% + 10%)' },
                    'blok1-pietro1-m11': { top: '20%', left: '0%', width: '20%', height: '40%', clipPath: 'polygon(27% 21%, 91% 21%, 91% 33%, 99% 32%, 99% 70%, 27% 70%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(0% + 10%)' },
                    'blok1-pietro1-m12': { top: '20%', left: '15%', width: '20%', height: '40%', clipPath: 'polygon(21% 21%, 78% 21%, 78% 32%, 82% 32%, 82% 70%, 25% 70%, 25% 31%, 21% 31%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(0% + 10%)' },
                    'blok1-pietro1-m13': { top: '20%', left: '25%', width: '20%', height: '40%', clipPath: 'polygon(39% 21%, 73.5% 21%, 73.5% 70%, 32% 70%, 32% 32%, 39% 32%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(40% + 10%)' },
                    'blok1-pietro1-m14': { top: '20%', left: '35%', width: '20%', height: '40%', clipPath: 'polygon(24% 21%, 73% 21%, 73% 32%, 75% 32%, 75% 70%, 24% 70%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(60% + 10%)' },
                    'blok1-pietro1-m15': { top: '40%', left: '35%', width: '20%', height: '40%', clipPath: 'polygon(39% 20%, 75% 20%, 75% 78%, 33% 78%, 33% 43%, 49% 43%, 49% 30%, 39% 30%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(20% + 10%)' },
                    'blok1-pietro1-m16': { top: '50%', left: '30%', width: '20%', height: '40%', clipPath: 'polygon(13% 22%, 31% 22%, 31% 5%, 75% 5%, 75% 18%, 57.4% 18%, 57.4% 53%, 13% 53%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(35% + 5%)' },
                    'blok1-pietro1-m33': { top: '50%', left: '70%', width: '20%', height: '40%', clipPath: 'polygon(35% 5%, 66.5% 5%, 66.5% 53.5%, 19.5% 53.5%, 19.5% 24%, 9% 24%, 9% 8%, 35% 8%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(40% + 10%)' },
                    'blok1-pietro1-m34': { top: '40%', left: '80%', width: '20%', height: '41.2%', clipPath: 'polygon(29% 30%, 17% 30%, 17% 76%, 74% 76%, 74% 19%, 29% 19%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-pietro1-m35': { top: '20%', left: '78%', width: '20%', height: '40%', clipPath: 'polygon(19% 21%, 84% 21%, 84% 70%, 19% 70%, 10% 70%, 10% 31%, 19% 31%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(20% + 10%)' },
                    'blok1-pietro1-m36': { top: '20%', left: '65%', width: '20%', height: '40%', clipPath: 'polygon(22% 22%, 79% 22%, 79% 32%, 75% 32%, 75% 70%, 19% 70%, 19% 31%, 22% 31%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(60% + 10%)' },
                    'blok1-pietro1-m37': { top: '20%', left: '55%', width: '20%', height: '40%', clipPath: 'polygon(27% 21%, 61.5% 21%, 61.5% 32%, 68% 32%, 68% 70%, 27% 70%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-pietro1-m38': { top: '20%', left: '45%', width: '20%', height: '40%', clipPath: 'polygon(27% 21%, 77% 21%, 77% 70%, 27% 70%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-pietro1-m39': { top: '40%', left: '45%', width: '20%', height: '41.5%', clipPath: 'polygon(62% 29%, 62% 19%, 25% 19%, 25% 76%, 68% 76%, 68% 41.5%, 50% 41.5%, 50% 29%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(50% + 2.5%)' },
                    'blok1-pietro1-m40': { top: '40%', left: '50%', width: '20%', height: '41.5%', clipPath: 'polygon(69% 29%, 27% 29%, 27% 41.5%, 43% 41.5%, 43% 76%, 87% 76%, 87% 46%, 69% 46%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(55% + 5%)' },

                    // BLOK 1 - PIĘTRO 2 (14 mieszkań) - ✅ POPRAWIONE ZGODNIE Z BAZĄ DANYCH
                    'blok1-pietro2-m17': { top: '39%', left: '15%', width: '20%', height: '39.4%', clipPath: 'polygon(9% 29%, 40% 29%, 40% 34%, 67% 34%, 67% 49%, 56% 49%, 56% 79%, 9% 79%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(40% + 10%)' },
                    'blok1-pietro2-m18': { top: '39%', left: '0%', width: '20%', height: '40%', clipPath: 'polygon(27% 20%, 71% 20%, 71% 29%, 84% 29%, 84% 78%, 27% 78%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(20% + 10%)' },
                    'blok1-pietro2-m19': { top: '19%', left: '0%', width: '20%', height: '40%', clipPath: 'polygon(27% 21%, 91% 21%, 91% 33%, 99% 32%, 99% 70%, 27% 70%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(0% + 10%)' },
                    'blok1-pietro2-m20': { top: '19%', left: '15%', width: '20%', height: '40%', clipPath: 'polygon(21% 21%, 78% 21%, 78% 32%, 82% 32%, 82% 70%, 25% 70%, 25% 31%, 21% 31%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(0% + 10%)' },
                    'blok1-pietro2-m21': { top: '19%', left: '25%', width: '20%', height: '40%', clipPath: 'polygon(39% 21%, 73.5% 21%, 73.5% 70%, 32% 70%, 32% 32%, 39% 32%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(40% + 10%)' },
                    'blok1-pietro2-m22': { top: '19%', left: '35%', width: '20%', height: '40%', clipPath: 'polygon(24% 21%, 73% 21%, 73% 32%, 75% 32%, 75% 70%, 24% 70%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(60% + 10%)' },
                    'blok1-pietro2-m23.24': { top: '40%', left: '30%', width: '25%', height: '40%', clipPath: 'polygon(25% 27%, 51% 27%, 51% 18%, 80% 18%, 80% 65%, 75% 65%, 75% 76%, 11% 76%, 11% 45%, 25% 45%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-pietro2-m41': { top: '48.7%', left: '70%', width: '20%', height: '40%', clipPath: 'polygon(35% 5%, 66.5% 5%, 66.5% 53.5%, 19.5% 53.5%, 19.5% 24%, 9% 24%, 9% 8%, 35% 8%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(40% + 10%)' },
                    'blok1-pietro2-m42': { top: '38.7%', left: '80%', width: '20%', height: '41.2%', clipPath: 'polygon(29% 30%, 17% 30%, 17% 76%, 74% 76%, 74% 19%, 29% 19%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-pietro2-m43': { top: '19%', left: '78%', width: '20%', height: '40%', clipPath: 'polygon(19% 21%, 84% 21%, 84% 70%, 19% 70%, 10% 70%, 10% 31%, 19% 31%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(20% + 10%)' },
                    'blok1-pietro2-m44': { top: '19%', left: '65%', width: '20%', height: '40%', clipPath: 'polygon(22% 21%, 79% 21%, 79% 32%, 75% 32%, 75% 70%, 19% 70%, 19% 31%, 22% 31%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(60% + 10%)' },
                    'blok1-pietro2-m45': { top: '19%', left: '55%', width: '20%', height: '40%', clipPath: 'polygon(27% 21%, 61.5% 21%, 61.5% 32%, 68% 32%, 68% 70%, 27% 70%)', labelTop: 'calc(0% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-pietro2-m46': { top: '19%', left: '45%', width: '20%', height: '40%', clipPath: 'polygon(27% 21%, 77% 21%, 77% 70%, 27% 70%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    'blok1-pietro2-m47.48': { top: '40%', left: '48%', width: '20%', height: '40%', clipPath: 'polygon(10% 17%, 47% 17%, 47% 27.5%, 79.5% 27.5%, 79.5% 44%, 97.5% 44%, 97.5% 75.5%, 16.6% 75.5%, 16.6% 65%, 10% 65%)', labelTop: 'calc(60% + 20%)', labelLeft: 'calc(80% + 10%)' },
                    
                    // BLOK 2 - PARTER (13 mieszkań) - ✅ POPRAWIONE ZGODNIE Z BAZĄ DANYCH
                    ...
                    'blok2-parter-m22': { top: '25%', left: '15%', width: '12%', height: '35%', clipPath: 'polygon(0% 0%, 100% 0%, 95% 100%, 5% 100%)', labelTop: 'calc(25% + 17.5%)', labelLeft: 'calc(15% + 6%)' },
                    'blok2-parter-m23': { top: '25%', left: '28%', width: '12%', height: '35%', clipPath: 'polygon(0% 0%, 100% 0%, 95% 100%, 5% 100%)', labelTop: 'calc(25% + 17.5%)', labelLeft: 'calc(28% + 6%)' },
                    'blok2-parter-m24': { top: '25%', left: '41%', width: '12%', height: '35%', clipPath: 'polygon(0% 0%, 100% 0%, 95% 100%, 5% 100%)', labelTop: 'calc(25% + 17.5%)', labelLeft: 'calc(41% + 6%)' },
                    'blok2-parter-m25': { top: '25%', left: '54%', width: '12%', height: '35%', clipPath: 'polygon(0% 0%, 100% 0%, 95% 100%, 5% 100%)', labelTop: 'calc(25% + 17.5%)', labelLeft: 'calc(54% + 6%)' },
                    'blok2-parter-m26': { top: '25%', left: '67%', width: '12%', height: '35%', clipPath: 'polygon(0% 0%, 100% 0%, 95% 100%, 5% 100%)', labelTop: 'calc(25% + 17.5%)', labelLeft: 'calc(67% + 6%)' },
                    'blok2-parter-m27': { top: '25%', left: '80%', width: '12%', height: '35%', clipPath: 'polygon(0% 0%, 100% 0%, 95% 100%, 5% 100%)', labelTop: 'calc(25% + 17.5%)', labelLeft: 'calc(80% + 6%)' },
